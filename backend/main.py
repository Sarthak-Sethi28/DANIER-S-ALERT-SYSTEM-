...
def _create_or_get_default_user(db: Session):
    """Fallback: create admin user if none exists (called during login as safety net)."""
    try:
        ...
        db.add(user)
        db.commit()
        print(f"✅ Admin user created on login fallback: username='{username}'")
        return user
    except Exception as e:
        db.rollback()
        print(f"⚠️ Error in _create_or_get_default_user: {e}")
        return None

def _sync_admin_user_on_startup(db: Session):
    """Called on startup: always ensures admin user exists and credentials match env vars.
    This guarantees the admin can ALWAYS login by setting APP_USERNAME / APP_PASSWORD
    environment variables on Render and redeploying."""
    try:
        username = os.getenv('APP_USERNAME', 'danier_admin')
        raw_password = os.getenv('APP_PASSWORD', 'danier2024')
        email = os.getenv('APP_USER_EMAIL', 'danieralertsystem@gmail.com')
        ...
        user = db.query(UserCredential).first()
        if user:
            # Always re-hash and update password from env vars so admin can always login
            new_salt = secrets.token_hex(16)
            new_hash = _hash_password(raw_password, new_salt)
            ...
            db.add(user)
            db.commit()
            print(f"✅ Admin credentials synced from env vars on startup: username='{username}'")
        else:
            ...
            db.add(user)
            db.commit()
            print(f"✅ Admin user created on startup: username='{username}'")
        return user
    except Exception as e:
        db.rollback()
        print(f"⚠️ Error syncing admin user on startup: {e}")
        return None
...
async def download_all_alerts():
    ...
    try:
        ...
        # Headers
        headers = ["Item Name", "Item Number", "Color", "Size", "Current Stock", "Required Threshold", "Shortage", "New Order", "Order Date", "Priority"]
        ...
        for item in all_alerts:
            ...
            for alert in alerts:
                ...
                # Order Date may be None if column not present; write empty string in that case
                order_date_value = alert.get("order_date", None)
                ws.cell(row=row, column=9, value=order_date_value if order_date_value is not None else "")
                ws.cell(row=row, column=10, value=priority)
...
async def warm_caches_on_startup():
    ...
    try:
        ...
        try:
            _sync_admin_user_on_startup(db)
            print("✅ Admin user synced from environment variables")
        finally:
            db.close()
